...
if (isKnownService || isBillingPage) {
  console.log('Vaultly active on:', host);
  ...
  const injectFloatingBanner = (serviceName) => {
    if (document.getElementById('vaultly-saver-banner')) return;

    const banner = document.createElement('div');
    banner.id = 'vaultly-saver-banner';
    banner.style.cssText = `
      ...
      animation: vaultly-slide-in 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    `;

    // Inject animation keyframes
    const style = document.createElement('style');
    style.innerHTML = `
      @keyframes vaultly-slide-in {
        from { transform: translateY(100px) scale(0.9); opacity: 0; }
        to { transform: translateY(0) scale(1); opacity: 1; }
      }
      @keyframes vaultly-fade-out {
    ...
    banner.innerHTML = `
      <div style="display: flex; align-items: center; gap: 8px;">
        <img src="${logoUrl}" style="width: 32px; height: 32px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.05); object-fit: contain; padding: 3px;" onerror="this.onerror=null; this.src='https://vaultly.onrender.com/favicon.png'">
        <div style="text-align: left;">
          <h4 style="margin: 0; font-size: 13px; font-weight: 800; color: white;">Save to Vaultly?</h4>
          <p style="margin: 2px 0 0; font-size: 11px; color: #9CA3AF;">Add ${serviceName} subscription</p>
        </div>
      </div>
      <div style="display: flex; gap: 8px; align-items: center;">
        <button id="vaultly-add-now" style="background: linear-gradient(135deg, #6C63FF, #FF007F); border: none; color: white; font-weight: bold; font-size: 11px; padding: 8px 14px; border-radius: 8px; cursor: pointer; transition: opacity 0.2s;">Add Now</button>
        <button id="vaultly-dismiss" style="background: transparent; border: 1px solid rgba(255,255,255,0.2); color: #D1D5DB; font-weight: 600; font-size: 11px; padding: 8px 12px; border-radius: 8px; cursor: pointer; transition: background 0.2s;">Dismiss</button>
      </div>
    `;

    document.body.appendChild(banner);

    document.getElementById('vaultly-dismiss').addEventListener('click', () => {
      banner.style.animation = 'vaultly-fade-out 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards';
      setTimeout(() => banner.remove(), 300);
    });

    document.getElementById('vaultly-add-now').addEventListener('click', () => {
      ...
      const btn = document.getElementById('vaultly-add-now');
